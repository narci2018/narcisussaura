...
export const ProbingChip: React.FC<{ startedAt: number; now: number }> = ({ startedAt, now }) => (
  <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[12px] font-medium border bg-blue-500/10 text-blue-300 border-blue-500/30">
 ...
 *
 * 卡片上同样有这一句,但卡片会被重排送走 —— 刚判死的沉到 98 张的末尾,用户眼睁睁
 * 看着那句话消失,这就是"提示一闪而过"。放在这里它就不依赖滚动位置了。
 */
  ...
  return (
    <div className="flex items-start gap-2 px-2.5 py-2 rounded-xl bg-[#12151f] border border-[#23293d]">
      <div className="flex-1 min-w-0">
        {nodeName ? (
          <div className="text-[12px] text-gray-500 truncate">{`「${nodeName}」的测活结论`}</div>
        ) : null}
      ...
      <button
        onClick={onDismiss}
        title="知道了"
        className="p-1 text-gray-500 active:text-gray-300 shrink-0"
      >
